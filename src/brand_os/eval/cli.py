...
def grade(
    text: str | None = typer.Argument(None, help="Text to grade (optional; use - or pipe stdin)"),
    ...
    # Resolve input text:
    # - Explicit "-" means read stdin.
    # - Missing TEXT reads stdin if piped; otherwise use a short sample.
    if text == "-":
        text = sys.stdin.read()
    elif text is None:
        if not sys.stdin.isatty():
            ...
            text = "Sample content for grading."

    # If no provider is explicitly configured and there are no API keys,
    # fall back to the mock provider so local eval commands still run.
    if not os.getenv("BRANDOPS_LLM_PROVIDER"):
        if not (os.getenv("GOOGLE_API_KEY") or os.getenv("ANTHROPIC_API_KEY")):
            os.environ["BRANDOPS_LLM_PROVIDER"] = "mock"
    ...
    result = grade_content(text, rubric=rubric, brand=brand)
...
def drift(
    ...
    if format == "table":
        consistent = "[green]Yes[/green]" if result.is_consistent else "[red]No[/red]"
        console.print(f"[bold]Consistent: {consistent}[/bold]")
...
def heal(
    ...
    if result["success"]:
        console.print(
            f"[green]Success![/green] Achieved {result['final_score']:.2f} "
            f"in {result['iterations']} iterations"
        )
    else:
        console.print(
            f"[yellow]Did not reach target.[/yellow] Final score: {result['final_score']:.2f}"
        )
